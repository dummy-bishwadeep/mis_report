...
logging_config["level"] = LoggingDetails.LOG_LEVEL
enable_traceback: bool = LoggingDetails.TRACEBACK_FLAG
...
def get_logger():
    ...
    log_formatter = (
        "%(asctime)s - %(levelname)-6s - [%(threadName)5s:%(funcName)5s():" + "" + "%(lineno)s] - %(message)s"
    )
    time_format = "%Y-%m-%d %H:%M:%S"
    formatter = logging.Formatter(log_formatter, time_format)

    for each_handler in logging_config["handlers"]:
        if each_handler["type"] in ["RotatingFileHandler"] and LoggingDetails.ENABLE_FILE_LOG:
        ...
        elif each_handler["type"] in ["StreamHandler"] and LoggingDetails.ENABLE_CONSOLE_LOG:
